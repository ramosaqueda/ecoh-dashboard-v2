...
const getEstadoBadge = (estado: string) => {
  switch (estado) {
    ...
    case 'terminado':
      return <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">Terminado</Badge>;
    default:
...
export default function TodoActividades() {
  ...
  // Estados para highlight individual
  const [highlightedActividad, setHighlightedActividad] = useState<Actividad | null>(null);
    ...
    try {
      ...
      if (userWithRole.ok) {
        ...
        if (foundUser) {
          ...
          console.log('✅ Usuario configurado:', foundUser.nombre || foundUser.email);
          if (!highlightId) {
      ...
      console.error('❌ Error fetchCurrentUser:', error);
  ...
  const fetchActividadById = async (id: number) => {
    ...
    try {
      ...
      console.error('❌ Error fetchActividadById:', error);
  ...
  const handleActivityClose = async () => {
    ...
    try {
      ...
      // Si estamos en modo highlight y es la misma actividad, actualizarla
      if (highlightedActividad && highlightedActividad.id === actividadToClose.id) {
        setHighlightedActividad({
          ...highlightedActividad,
          estado: 'terminado',
          glosa_cierre: glosaCierre.trim() || undefined
        });
      }
  ...
  const handleBackToActividades = () => {
    router.push('/dashboard/actividades');
  };

  const handleShowAllTasks = () => {
    // Limpiar el parámetro highlight y mostrar todas las tareas
    const newUrl = new URL(window.location.href);
    newUrl.searchParams.delete('highlight');
    router.replace(newUrl.pathname + newUrl.search);
    setHighlightedActividad(null);
    setHighlightError(null);
  };
  ...
  useEffect(() => {
    if (highlightId) {
      fetchActividadById(parseInt(highlightId));
    }
  }, [highlightId]);
  ...
  // Si hay un highlight, mostrar solo esa actividad
  if (highlightId) {
    ...
    if (highlightError || !highlightedActividad) {
      return (
        <PageContainer>
          <div className="space-y-6">
            <div className="flex items-center justify-between">
    ...
    return (
      <PageContainer>
        <div className="space-y-6">
              ...
              <div className="flex items-start justify-between">
                <div>
                  <CardTitle className="text-xl">
                    {highlightedActividad.tipoActividad.nombre}
                  </CardTitle>
                  <p className="text-sm text-gray-600 mt-1">
                    Actividad ID: {highlightedActividad.id}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {getEstadoBadge(highlightedActividad.estado)}
                  {/* Botón removido - solo queda el de abajo */}
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="ml-6 space-y-1">
                    {highlightedActividad.usuario && (
                      <p className="text-sm">
                        <strong>Creado por:</strong> {highlightedActividad.usuario.nombre || highlightedActividad.usuario.email}
                      </p>
                    )}
                    {highlightedActividad.usuarioAsignado && (
                      <p className="text-sm">
                        <strong>Asignado a:</strong> {highlightedActividad.usuarioAsignado.nombre || highlightedActividad.usuarioAsignado.email}
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <div className="ml-6">
                    <p className="text-sm">{formatDate(highlightedActividad.fechaInicio)}</p>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="ml-6">
                    <p className={`text-sm ${getPriorityColor(highlightedActividad.fechaTermino)}`}>
                      {formatDate(highlightedActividad.fechaTermino)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              {/* Observaciones */}
              {highlightedActividad.observacion && (
                <div className="space-y-2">
                  <div className="flex items-center text-sm font-medium text-gray-700">
                    <FileText className="mr-2 h-4 w-4" />
                    Observaciones
                  ...
                  <div className="ml-6 p-3 bg-gray-50 rounded-md">
                    <p className="text-sm whitespace-pre-wrap">{highlightedActividad.observacion}</p>
                  </div>
                </div>
              )}

              {/* Glosa de cierre si existe */}
              {highlightedActividad.glosa_cierre && (
                <div className="space-y-2">
                  <div className="flex items-center text-sm font-medium text-gray-700">
                  ...
                  <div className="ml-6 p-3 bg-green-50 rounded-md">
                    <p className="text-sm whitespace-pre-wrap">{highlightedActividad.glosa_cierre}</p>
                  </div>
                </div>
              )}
              ...
              <div className="flex justify-end pt-4 border-t">
                {highlightedActividad.estado !== 'terminado' && (
                  <button
                    onClick={() => {
                      console.log('🔥 COMPLETAR: Botón clickeado!');
                      console.log('📝 Actividad ID:', highlightedActividad.id);
                      console.log('📝 Estado formulario antes:', showCloseDialog);
                      setActividadToClose(highlightedActividad);
                      setGlosaCierre(highlightedActividad.glosa_cierre || '');
                      setShowCloseDialog(true);
                      console.log('📝 Estado formulario después: TRUE');
                    }}
                    className="bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-medium flex items-center gap-2 transition-colors"
                  >
                    <CheckCircle2 className="h-5 w-5" />
                    {showCloseDialog ? 'Cancelar Completar' : 'Completar Actividad'}
                  </button>
                )}
              </div>
            </CardContent>
          ...
          {/* FORMULARIO DE COMPLETAR - Directamente en la página */}
          {showCloseDialog && highlightedActividad && (
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3 pt-4">
                    <button
                      ...
                      disabled={isUpdating}
                      className="flex-1 px-4 py-3 border border-gray-300 rounded text-gray-700 hover:bg-gray-50 disabled:opacity-50 font-medium"
                    >
                      Cancelar
                    </button>
                    <button
                      onClick={() => {
                        console.log('🚀 Ejecutando handleActivityClose...');
                        handleActivityClose();
                      }}
                      disabled={isUpdating}
                      className="flex-1 px-4 py-3 bg-green-600 hover:bg-green-700 text-white rounded disabled:opacity-50 flex items-center justify-center gap-2 font-medium"
                    >
                      {isUpdating ? (
                        <>
  ...
  return (
    <PageContainer>
      <div className="space-y-6">
        ...
        {showCloseDialog && (
          <div className="fixed top-4 right-4 bg-green-500 text-white p-3 rounded z-[10000] font-bold">
