...
import { Plus, ArrowLeft, Trash2, Loader2, Calendar, Clock, Image as ImageIcon, FileText } from 'lucide-react';
...
export default function TimelinePage({
  ...
  const scrollToHito = (date: string) => {
    ...
    if (index !== -1) {
      const elementId = `hito-${sortedHitos[index].id}`;
  ...
  const getIcon = (hito: TimelineHito) => {
    // Si hay un icono personalizado (string), podríamos mapearlo aquí si tuviéramos un mapa de iconos
    // Por ahora retornamos iconos por defecto según el contenido
    if (hito.imagenUrl) return <ImageIcon />;
    if (hito.descripcion) return <FileText />;
    return <Clock />;
  };
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="space-y-6 mb-8">
          ...
          {/* Timeline Navigation */}
          {hitos.length > 0 && (
            <div className="bg-white rounded-lg shadow-sm p-4 sticky top-4 z-50">
              <TimelineNav
              ...
              <TabsContent value="chrono" className="w-full">
                <div className="bg-gray-50 rounded-lg p-6">
                  <VerticalTimeline lineColor="#e5e7eb">
                    {hitos
                      .sort((a, b) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime()) // Orden descendente para timeline vertical
                      .map((hito) => (
                        <VerticalTimelineElement
                          key={hito.id}
                          id={`hito-${hito.id}`}
                          className="vertical-timeline-element--work"
                          contentStyle={{ background: '#fff', color: '#000', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1)', borderRadius: '0.5rem', border: '1px solid #e5e7eb' }}
                          contentArrowStyle={{ borderRight: '7px solid  #fff' }}
                          date={formatDateTime(hito.fecha)}
                          dateClassName="text-gray-600 font-medium mx-4"
                        ...
                        >
                          <div className="group relative">
                            ...
                            <h3 className="vertical-timeline-element-title text-lg font-bold text-gray-900 mb-2">
                              {hito.titulo}
                            </h3>
                            ...
                            {hito.descripcion && (
                              <p className="vertical-timeline-element-description text-gray-600 whitespace-pre-wrap">
                                {hito.descripcion}
                              </p>
                            )}
                          </div>
              ...
              <TabsContent value="list">
                <div className="max-w-4xl mx-auto space-y-4 pb-10">
                  {hitos.sort((a, b) =>
                    ...
                    <div key={hito.id} id={`hito-list-${hito.id}`} className="w-full">
                      <EnhancedHitoCard
                        hito={hito}
