...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('Error en GET /api/estados-causa:', error);
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error('Error en POST /api/estados-causa:', error);
...
export async function PUT(req: NextRequest) {
  try {
    ...
    console.error('Error en PUT /api/estados-causa:', error);
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const causasAsociadas = await prisma.causa.count({
      where: { estadoCausaId: Number(id) }
    });

    if (causasAsociadas > 0) {
      return NextResponse.json(
        { 
          message: `No se puede eliminar. Hay ${causasAsociadas} causa(s) asociada(s) a este estado.`,
          causasAsociadas 
        },
        { status: 409 }
      );
    ...
    return NextResponse.json(
      { message: 'Estado de causa eliminado correctamente' }
    );
  } catch (error) {
    console.error('Error en DELETE /api/estados-causa:', error);
