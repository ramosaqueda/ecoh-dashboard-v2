...
const FICHAB_ENDPOINT = '/fichab/Caso/relato';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[FICHAB] Consultando Caso/relato para RUC: ${ruc}`);
    ...
    if (response.status >= 400) {
      console.error(`Error FICHAB Relato: ${response.status} ${response.statusText}`);
      return NextResponse.json(
        { message: `Error del servicio FICHAB Relato: ${response.status}` },
    ...
    try {
      ...
      return NextResponse.json({ 
        success: true, 
        data: { bloque: text, raw: true }
      });
    }

  } catch (error) {
    console.error('Error en proxy FICHAB Caso Relato:', error);
    return NextResponse.json(
      { message: 'Error interno del servidor al consultar FICHAB Relato' },
      { status: 500 }
    );
  }
}
