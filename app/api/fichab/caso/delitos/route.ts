...
const FICHAB_ENDPOINT = '/fichab/Caso/delitos';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[FICHAB] Consultando Caso/delitos para RUC: ${ruc}`);
    ...
    if (response.status >= 400) {
      console.error(`Error FICHAB Delitos: ${response.status} ${response.statusText}`);
      return NextResponse.json(
        { message: `Error del servicio FICHAB Delitos: ${response.status}` },
    ...
    try {
      ...
      console.warn('FICHAB Delitos devolvió algo no JSON:', text.substring(0, 100));
      return NextResponse.json({ 
        success: true, 
    ...
    console.error('Error en proxy FICHAB Caso Delitos:', error);
    return NextResponse.json(
      { message: 'Error interno del servidor al consultar FICHAB Delitos' },
      { status: 500 }
    );
  }
}
